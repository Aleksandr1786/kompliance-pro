...
const SECTIONS = {
  OT: [
    ...
    // OT_PASF добавлен 12.07.2026 — раньше такой записи не было вообще,
    // хотя папка на диске ('Раздел 9. ПАСФ', см. generator.js dirs.d10)
    // существовала. Документы ПАСФ падали в «Прочее» в приложении —
    // просто это никто не заметил, пока не тестировали клиента с ПАСФ.
    { id:'OT_PASF',      order:9, folder:'Раздел 9. ПАСФ',                                         title:'ПАСФ',                  icon:'life-buoy' },
    // Новый раздел по просьбе Александра 12.07.2026 — План мероприятий и
    // График периодических мероприятий раньше жили в «Приказах» просто
    // из-за исторической нумерации функций-генераторов (gen_01_10/11).
    { id:'OT_PLANS',     order:10, folder:'Раздел 10. Планы мероприятий',                          title:'Планы',                 icon:'calendar' },
...
const FILE_SECTION = {
  ...
  // Раздел 5. Журналы учёта
  // Позиции ниже (кроме прочего) добавлены 12.07.2026 — реально пишутся
  // generator.js в dirs.d6 = 'Раздел 5. Журналы учёта', но отсутствовали
  // здесь и падали в «Прочее» в приложении, при этом на диске лежали
  // правильно (два параллельных, не связанных между собой источника
  // правды — см. комментарий в шапке файла). Сверить с реальными именами
  // после генерации у клиента с максимальным набором условных документов.
  'Журнал контроля аптечки первой помощи.docx': 'OT_JOURNALS',
  'Журнал учёта микротравм.docx': 'OT_JOURNALS',
  // Подтверждено сверкой с generator.js (gen_06_01/gen_06_02) — 12.07.2026
  'Журнал регистрации вводного инструктажа.docx': 'OT_JOURNALS',
  'Журнал регистрации инструктажа на рабочем месте.docx': 'OT_JOURNALS',
  // Не найдены в доступных файлах — оставлено по тематической аналогии,
  // не подтверждено сверкой с кодом:
  'План мероприятий по охране труда.docx': 'OT_PLANS',
  'График периодических мероприятий.docx': 'OT_PLANS',
  // Раздел 3. Электробезопасность — компаньон уже существующей записи
  'Журнал учёта присвоения I группы электробезопасности.docx': 'OT_ELECTRO',
  // Раздел 7. Протоколы обучения — по аналогии с уже существующим
  // «Журнал регистрации противопожарных инструктажей» (тот же жанр:
  // журнал регистрации проведённого инструктажа)
  // ИСПРАВЛЕНО 12.07.2026: сверил с generator.js (gen_06_01/02) — эти два
  // документа реально пишутся в dirs.d6 = OT_JOURNALS, а не в d8 (Протоколы),
  // как я предполагал по аналогии раньше. Перенёс в блок выше.
  ...
  'ВУ-07 Сведения о принятых и уволенных военнообязанных.docx': 'VU_NOTICES',
  'ВУ-08 Листок сообщения об изменениях сведений военнообязанного.docx': 'VU_NOTICES',
...
/**
 * Обратный поиск: определяет раздел по имени папки на диске (родительская
 * папка файла). Это и есть настоящий источник правды — папка на диске
 * создаётся ОДИН раз, в момент генерации (см. generator.js, dirs.dN), и
 * именно туда попадает конкретный файл. FILE_SECTION ниже (соответствие
 * по имени файла) — это отдельный, вручную поддерживаемый словарь для тех
 * же целей, который на практике отстаёт от реального набора генераторов
 * (см. историю бага 12.07.2026). doc-generation.js теперь сохраняет
 * section в записи документа при генерации через эту функцию — FILE_SECTION
 * остаётся только как резервный путь для документов, сгенерированных до
 * этой правки (в их записи в базе поле section ещё не заполнено).
 * @param {string} module
 * @param {string} folderName — имя папки-родителя файла (basename, без пути)
 * @returns {object|null}
 */
function sectionByFolder(module, folderName) {
  const list = SECTIONS[module] || [];
  return list.find(s => s.folder === folderName) || null;
}
...
function groupBySections(module, docs) {
  ...
  const buckets = {};
  for (const d of (docs || [])) {
    // Приоритет — реальный раздел, записанный при генерации (doc-generation.js,
    // 12.07.2026). Старые документы (сгенерированные до этой правки) поля
    // section не имеют — для них резервный путь по имени файла, как раньше.
    const sid = d.section || sectionOf(module, d.name || d.filename || '');
