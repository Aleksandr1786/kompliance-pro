'use strict';
// КомплаенсПро icons.js — библиотека SVG иконок (Lucide-стиль)
...
const ICON_PATHS = {
  ...
  'heart-pulse': `<path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"/><path d="M3.22 8.5h3.16l1.62-2.5 2 5 1.62-2.5h5.16"/>`,
 ...
 * @param {string} name  — имя иконки из ICON_PATHS
 * @param {number} size  — размер в пикселях (default 16)
 * @param {string} style — дополнительный CSS стиль
 */
function ic(name, size=16, style='') {
  const d = ICON_PATHS[name];
  if (!d) return '';
  return `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:middle;flex-shrink:0;${style}">${d}</svg>`;
}
